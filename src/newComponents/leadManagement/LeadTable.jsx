  ...
  const EditModal = ({ lead, onClose, onSave }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[95vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
          <div className="p-4 border-b flex justify-between items-center">
            <h2 className="text-lg font-bold text-gray-900">Edit Lead</h2>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-900 font-bold">×</button>
          </div>
          <div className="flex-1 overflow-y-auto p-4">
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={key} className="h-[4.5rem]">
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">{key.replace(/([A-Z])/g, " $1")}</label>
                  <input
                    type={type}
                    name={key}
                    value={formData[key] || ""}
                    onChange={(e) => setFormData({ ...formData, [key]: e.target.value })}
                    className="w-full px-3 py-1.5 border rounded-lg text-sm outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 border-gray-300 hover:border-gray-400"
                  />
                </div>
              ))}

              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">Expected Travel Date</label>
                <input
                  type="date"
                  name="expectedTravelDate"
              ...
              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">No. of Days</label>
                <select
                  name="noOfDays"
                  value={formData.noOfDays || ""}
                ...
                >
                  ...
                  {tripDurations.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}
                </select>
              </div>

              {formData.noOfDays === "Others" && (
                <div className="h-[4.5rem]">
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Custom Days</label>
                  <input
                    name="customNoOfDays"
                    value={formData.customNoOfDays || ""}
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.placesToCoverArray.map((p, i) => (
                    <span key={`${p}-${i}`} className="bg-blue-100 text-blue-700 px-2 py-1 rounded flex items-center gap-1 text-sm">
                      {p}
                      <button type="button" onClick={() => handleRemovePlace(i)}>x</button>
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.childAges.map((age, i) => (
                    <span key={`${age}-${i}`} className="bg-green-100 text-green-700 px-2 py-1 rounded flex items-center gap-1 text-sm">
                      {age}
                      <button type="button" onClick={() => handleRemoveChildAge(i)}>x</button>
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">Lead Source</label>
                <select
                  name="leadSource"
                  value={formData.leadSource || ""}
              ...
              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">Lead Type</label>
                <select
                  name="leadType"
                  value={formData.leadType || ""}
                ...
                >
                  ...
                  {leadTypes.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}
                </select>
              </div>

              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">Trip Type</label>
                <select
                  name="tripType"
                ...
                >
                  ...
                  {tripTypes.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}
                </select>
              </div>

              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">Lead Status</label>
                <select
                  name="leadStatus"
                  value={formData.leadStatus || "Cold"}
                  ...
                  className="w-full px-3 py-1.5 border rounded-lg text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 border-gray-300 hover:border-gray-400"
                >
                  {leadStatuses.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}
                </select>
              </div>

              <div className="h-[4.5rem]">
                <label className="block text-xs font-medium text-gray-700 mb-0.5">Group Number</label>
                <input
                  name="groupNumber"
                  value={formData.groupNumber || ""}
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 border-gray-300 hover:border-gray-400"
                />
              </div>
  ...
  const ViewModal = ({ lead, onClose }) => {
    ...
    const fields = [
      ...
      { label: "No. of Days", value: lead.noOfDays },
      {
        ...
        value:
          Array.isArray(lead.placesToCover)
            ? lead.placesToCover.join(", ")
            : lead.placesToCover
      },
      ...
      {
        label: "Child Ages",
        value: Array.isArray(lead.childAges)
          ? lead.childAges.join(", ")
          : lead.childAges
      },
